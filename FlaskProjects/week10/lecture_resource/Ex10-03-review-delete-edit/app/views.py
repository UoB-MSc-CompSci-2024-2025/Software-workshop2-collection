...
def product(id):
    ...
    if current_user.is_authenticated and review_form.validate_on_submit():
        ...
        if review_form.edit.data == '':
            review = Review(user=current_user, stars=int(review_form.stars.data), text=text)
            product.reviews.append(review)
        else:
            (edit_pid, edit_uid) = tuple(int(x) for x in review_form.edit.data.split(','))
            if id != edit_pid:
                # this should never happen but, if it does, it is an indication that a hacker is at work
                flash('Error: your review form submission does not match the product you were viewing')
                return redirect(url_for('product', id=id))
            review = db.session.get(Review, (edit_pid, edit_uid))
            review.stars = int(review_form.stars.data)
            review.text = text
        try:
            db.session.commit()
    ...
    return render_template('product.html', title='Product', form=review_form, choose_form=choose_form,
                           product=product, avg_stars=avg_stars)

@app.route("/edit_review", methods=['POST'])
def edit_review():
    choose_form = ChooseForm()
    if choose_form.validate_on_submit():
        (product_id, user_id) = tuple(int(x) for x in choose_form.choice.data.split(','))
        review = db.session.get(Review, (product_id, user_id))
        review_form = ReviewForm(edit=str(product_id) + "," + str(user_id), stars=review.stars, text=review.text)
        q = db.select(sa.func.avg(Review.stars)).where(Review.product == review.product)
        avg_stars = db.session.scalar(q)
        return render_template('product.html', title="Product", form=review_form, choose_form=choose_form,
                               product=review.product, avg_stars=avg_stars)
    return redirect(url_for('products'))
