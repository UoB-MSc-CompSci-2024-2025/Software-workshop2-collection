        ...
        if form.file.data:
            ...
            try:
                ...
                if error_count > 0:
                    raise ValueError
                if num_duplicates > 0:
                    flash(f'Warning: {num_duplicates} duplicate rows removed', 'warning')
                flash(f'File Uploaded', 'success')
                ...
                mem_bytes.seek(0)
                return send_file(mem_bytes, as_attachment=True, download_name='gen_contacts.csv', mimetype='text/csv')
            except Exception as err:
                flash(f'File upload failed.'
